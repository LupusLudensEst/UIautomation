...
- `I expect that element "([^"]*)?" is( not)* displayed` <br>Check if a certain element is visible
- `I expect that element "([^"]*)?" becomes( not)* displayed` <br>Check if a certain element becomes visible
...
- `I wait on element "([^"]*)?"( for (\d+)ms)*( to( not)* (be checked|be enabled|be selected|be displayed|contain a text|contain a value|exist))*` <br>Wait for an element to be checked, enabled, selected, visible, contain a certain value or text or to exist
